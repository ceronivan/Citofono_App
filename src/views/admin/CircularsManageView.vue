...
function openEdit(c: Circular) {
  ...
  form.value = { title: c.title, body: c.body }
  attachment.value = null
  attachmentPreview.value = c.attachmentUrl ?? null
  dialog.value = true
}
...
function onFileChange(e: Event) {
  ...
  if (file && file.type !== 'application/pdf') attachmentPreview.value = URL.createObjectURL(file)
}
...
const deletingItem = ref<Circular | null>(null)
const deleting = ref(false)

function openDelete(c: Circular) {
...
<template>
  <div class="manage-screen">
    <div class="manage-header">
      <BackBtn />
      <h1 class="manage-title">Circulares</h1>
    ...
    <div class="manage-body">
      ...
      <EmptyState
        ...
        message="No hay circulares publicadas"
      />

      <div v-else class="items-card">
        <div
          v-for="(c, idx) in store.items"
          :key="c.id"
          class="item-row"
        ...
        >
          <div class="item-icon">
            <VIcon
              :icon="c.attachmentType === 'pdf' ? 'mdi-file-pdf-box' : c.hasAttachment ? 'mdi-file-image-outline' : 'mdi-file-document-outline'"
              size="18"
              color="#4338CA"
            />
          </div>
          <div class="item-info">
            ...
            <span class="item-date">{{ fmt(c.publishedAt) }}</span>
          </div>
          <div class="item-actions">
            <button class="action-btn" @click="openEdit(c)">
              <VIcon icon="mdi-pencil-outline" size="16" />
            </button>
            <button class="action-btn action-btn--danger" @click="openDelete(c)">
    ...
    <VDialog v-model="dialog" max-width="520" :persistent="saving">
      <VCard rounded="xl">
        <div class="dialog-header">
          <span class="dialog-title">{{ editingId ? 'Editar circular' : 'Nueva circular' }}</span>
        ...
        <div class="dialog-body">
          ...
          <VTextarea v-model="form.body" label="Contenido *" rows="5" auto-grow class="mb-3" />
          ...
          <img
            v-if="attachmentPreview && attachment?.type !== 'application/pdf'"
    ...
    <VDialog v-model="deleteDialog" max-width="360" :persistent="deleting">
      <VCard rounded="xl">
        <div class="dialog-body">
          ...
          <p class="delete-title">¿Eliminar circular?</p>
        ...
        <div class="dialog-footer">
          ...
          <button class="footer-btn footer-btn--danger" :disabled="deleting" @click="handleDelete">
...
.item-date { font-size: 11px; color: var(--color-text-tertiary); }
...
.dialog-title { font-size: 17px; font-weight: 700; color: var(--color-text-primary); }
...
.dialog-footer { display: flex; gap: 10px; padding: 0 20px 20px; }
...
.footer-btn--primary { background: var(--color-admin); color: white; }
...
.delete-title { font-size: 17px; font-weight: 700; text-align: center; margin: 0 0 8px; }
.delete-subtitle { font-size: 14px; color: var(--color-text-secondary); text-align: center; margin: 0; line-height: 1.5; }
