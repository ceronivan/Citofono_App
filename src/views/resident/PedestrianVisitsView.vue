...
const visits = computed(() => store.items.filter(v => v.type === 'pedestrian'))
const fmt = (ts: any) => ts?.toDate ? dayjs(ts.toDate()).format('DD/MM/YYYY HH:mm') : '—'
</script>

<template>
  <div>
    <ScreenHeader title="Visitas Peatonales" />
    <VContainer class="py-4">
      <LoadingSpinner v-if="store.loading" />
      <EmptyState v-else-if="!visits.length" icon="mdi-walk" message="No tienes visitas registradas" />
      <VList v-else rounded="lg" border>
        <template v-for="(v, i) in visits" :key="v.id">
          <VListItem>
            <template #prepend>
              <AppAvatar :src="v.visitorPhotoUrl" icon="mdi-account-outline" size="48" class="mr-3" />
            </template>
            <VListItemTitle class="font-weight-semibold">{{ v.visitorName }}</VListItemTitle>
